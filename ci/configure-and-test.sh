...
for variable in \
  LIBPKGSOURCE_SOURCE LIBPKGCATALOG_SOURCE LIBPKGSTATE_SOURCE \
  LIBPKGRESOLVE_SOURCE LIBPKGBUILD_SOURCE LIBPKGTRANSACTION_SOURCE \
  LIBPKGCHECK_SOURCE LIBPKGEXEC_SOURCE
do
  eval "value=\${$variable:-}"
...
setup_dependency "$LIBPKGTRANSACTION_SOURCE" "$build/libpkgtransaction"
setup_dependency "$LIBPKGCHECK_SOURCE" "$build/libpkgcheck"
...
consumer_flags=$(PKG_CONFIG_PATH="$consumer_pkgconfig" pkg-config --cflags libpkgcheck-exec)
if [ "$link_mode" = static ]; then
  consumer_libs=$(PKG_CONFIG_PATH="$consumer_pkgconfig" pkg-config --static --libs libpkgcheck-exec)
else
  consumer_libs=$(PKG_CONFIG_PATH="$consumer_pkgconfig" pkg-config --libs libpkgcheck-exec)
fi
# shellcheck disable=SC2086
"${CXX:-c++}" -std=c++17 -Wall -Wextra -Wpedantic -Werror \
  $consumer_flags "$consumer" -o "$consumer_bin" $consumer_libs
LD_LIBRARY_PATH="$build/install/lib:$dependency_prefix/lib${LD_LIBRARY_PATH:+:$LD_LIBRARY_PATH}" "$consumer_bin"
