...
public:
  [[nodiscard]] static admitted_check_session admit(pkgcheck::check_request request, source_tree source, checked_package_tree package, std::vector<package_input_tree> inputs, session_paths paths, execution_identity identity, pkgexec::resource_limits limits = pkgexec::resource_limits::make());
  ...
  [[nodiscard]] const std::vector<package_input_tree>& inputs() const noexcept;
...
private:
  admitted_check_session(pkgcheck::check_request, source_tree, checked_package_tree, std::vector<package_input_tree>, session_paths, execution_identity, pkgexec::resource_limits);
  pkgcheck::check_request request_; source_tree source_; checked_package_tree package_; std::vector<package_input_tree> inputs_; session_paths paths_; execution_identity identity_; pkgexec::resource_limits limits_;
};
struct prepared_execution final { pkgexec::execution_request request; pkgexec::execution_resources resources; };
...
private:
  check_execution_result(pkgexec::execution_result, pkgcheck::check_result);
  pkgexec::execution_result execution_; pkgcheck::check_result check_;
  friend check_execution_result execute(const admitted_check_session&, pkgexec::execution_backend&);
};
}
